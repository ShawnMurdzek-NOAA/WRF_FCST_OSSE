...
if [ -r ${DATAHOME_BK}/wrfinput_d01 ]; then
  ${ECHO} " Cycled run using ${DATAHOME_BK}/wrfinput_d01"
  cp ${DATAHOME_BK}/wrfinput_d01 ./wrf_inout
  ${ECHO} " Cycle ${YYYYMMDDHH}: GSI background=${DATAHOME_BK}/wrfinput_d01" >> ${logfile}
...
if [ -r "${DATAROOT}/surface/wrfout_sfc_may" ]; then
  echo "cycle Surface fields based on ${DATAROOT}/surface/wrfout_sfc_may "
  ${LN} -s ${DATAROOT}/surface/wrfout_sfc_may ./wrfout_d01_save
...
# No time matched HRRR land surface file available
  ...
  ${ECHO} "${DATAROOT}/surface/wrfout_sfc_may does not exist!!"
