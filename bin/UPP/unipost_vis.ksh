...
if [ "${MODEL}" == "RAP" ]; then
  export POST=${EXE_ROOT}/unipost.exe_NEW_noSMOKEvis
  export CORE=RAPR
...
workdir=${DATAHOME}/${FCST_TIME}_vis
${RM} -rf ${workdir}
${MKDIR} -p ${workdir}
cd ${workdir}
...
ln -s ${STATIC_DIR}/postcntrl_hrrrx_vis.xml postcntrl.xml
ln -s ${STATIC_DIR}/postxconfig-NT-hrrrx-vis.txt postxconfig-NT.txt
if [ "${MODEL}" == "RAP" ]; then
...
${MV} ${workdir}/WRFPRS.GrbF${FCST_TIME}.new ${workdir}/wrfprsVIS_hrconus_${FCST_TIME}.grib2

# Append entire wrftwo to wrfnat
${CAT} WRFNAT.GrbF${FCST_TIME} WRFTWO.GrbF${FCST_TIME} > ${workdir}/WRFNAT.GrbF${FCST_TIME}.new
${MV} WRFNAT.GrbF${FCST_TIME}.new ${workdir}/wrfnatVIS_hrconus_${FCST_TIME}.grib2

${MV} ${workdir}/WRFTWO.GrbF${FCST_TIME} ${workdir}/wrftwoVIS_hrconus_${FCST_TIME}.grib2

# Check to make sure all Post  output files were produced
if [ ! -s "${workdir}/wrfprsVIS_hrconus_${FCST_TIME}.grib2" ]; then
  ${ECHO} "unipost crashed! wrfprsVIS_hrconus_${FCST_TIME}.grib2 is missing"
  exit 1
fi
if [ ! -s "${workdir}/wrftwoVIS_hrconus_${FCST_TIME}.grib2" ]; then
  ${ECHO} "unipost crashed! wrftwoVIS_hrconus_${FCST_TIME}.grib2 is missing"
  exit 1
fi
if [ ! -s "${workdir}/wrfnatVIS_hrconus_${FCST_TIME}.grib2" ]; then
  ${ECHO} "unipost crashed! wrfnatVIS_hrconus_${FCST_TIME}.grib2 is missing"
  exit 1
fi

# Move the output files to postprd
${MV} ${workdir}/wrftwoVIS_hrconus_${FCST_TIME}.grib2 ${DATAHOME}
${RM} -rf ${workdir}
