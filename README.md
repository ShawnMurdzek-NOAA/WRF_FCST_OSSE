...
8. NOTE: To start the initial spinup cycle, the `wrf_arw_short`, `wrf_arw_long`, and `HRRR_soilsave_01` tasks for all initialization hours for that spinup period (i.e., 03-08 or 15-20 UTC) must be manually completed, because these tasks are dependencies for the `gsi_hyb_spinup` task.
